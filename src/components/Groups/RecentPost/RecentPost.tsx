...
import { useGetAllGroupPost, usePostVote } from "@/hooks/post.hook";
import { toast } from "sonner";
import InfiniteScroll from "react-infinite-scroll-component";
import InfiniteScrollLoader from "@/components/Loader/InfiniteScrollLoader/InfiniteScrollLoader";

const RecentPost = ({ currentUserId }: { currentUserId: string }) => {
  // const contentRef = useRef<HTMLDivElement>(null);
  const { mutate: addComment, isPending } = useAddComment();
  const [userId, setUserId] = useState("");
  const { data, isLoading } = useGetAllGroupPost();
  ...
  return (
    <div>
      {data?.data?.slice(0, 3).map((item: TPost) => (
        <div
          key={item?._id}
